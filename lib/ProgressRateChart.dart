...
class ProgressRateChart extends StatefulWidget {
  const ProgressRateChart({super.key});

  @override
  State<ProgressRateChart> createState() => _ProgressRateChartState();
}

class _ProgressRateChartState extends State<ProgressRateChart>
    with TickerProviderStateMixin {
  final amber = const Color.fromRGBO(255, 192, 29, 1);

  Widget container(Habit habit, double totalBarWidth) {
    double width;
    if (habit.progressRate != 0) {
      width = habit.progressRate / 100 * totalBarWidth;
    } else {
      width = 5;
    }
  ...
  Ring ringChart(List<Habit> habitlist, BuildContext context, int i,
    ...
    return Ring(
      percent: habitlist[i].progressRate.toDouble(),
